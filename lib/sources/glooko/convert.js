...
function generate_nightscout_entries(batch, timestampDelta) {
  ...
  if (cgmReadings) {
    ...
    */
      ...
      entry.type = 'sgv';
      entry.sgv = element.y;
      entry.device = "ns-connect from glooko";
      entry.dateString = new Date(f_date + timestampDelta).toISOString( );
      entry.date = (element.x * 1000 + timestampDelta);
      //console.log('Timestamp delta: ', timestampDelta);
      console.log('entry.date : ', entry.date);
      //entry.utcOffset = timestampDelta / (1000*60*60);
      entry.direction = undefined; // TODO: we currently cannot obtain the direction / trend from glooko
      // offset in Minutes
      //entry.direction;
      entries.push(entry);
    })
